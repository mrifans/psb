              ...
              <table id="order-listing" class="table table-striped" data-name="cool-table">
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td>{{$b->pai}}</td>
                    <td>{{$b->rata2}}</td>
                    <td><form action="{{url('kelulusan')}}" method="post" >
                        @csrf
                      <input name="no_daftar" type="hidden" value="{{$b->no_daftar}}">
                      <input name="ket" type="hidden" value="{{$b->pil1}}">
                      <button class="btn btn-primary" type="submit">Pilihan 1</button>
                      </form>||<form action="{{url('kelulusan')}}" method="post" >
                        @csrf
                      <input name="no_daftar" type="hidden" value="{{$b->no_daftar}}">
                      <input name="ket" type="hidden" value="{{$b->pil2}}">
