...
class AdminController extends Controller
{
    ...
    public function index()
    {
        ...
        $tahap2pa = DB::table('siswa1')->where([
            ['is_mukim', 1],
            ['jk', 'L'],
            ['no_mukim', '!=',null],
            ['tgl_bayar', '>=','2020-06-06 13:00:00']
        ])->count();
        $tahap2pi = DB::table('siswa1')->where([
            ['is_mukim', 1],
            ['jk', 'P'],
            ['no_mukim', '!=',null],
            ['tgl_bayar', '>=','2020-06-06 13:00:00']
        ])->count();
        $tahap2 = DB::table('siswa1')->where([
            ['is_mukim', 1],
            ['no_mukim', '!=',null],
            ['tgl_bayar', '>=','2020-06-06 13:00:00']
        ])->count();
        ...
        if(Auth()->user()->id_user == 4){
            ...
            // $lulus1x = DB::table('kelulusan')->select('no_daftar')->where('tgl_lulus','01-07-2020');
            $tahap2x = DB::table('user_jawaban')->select('user_id')->wheredate('created_at','2020-07-01');
            ...
            $tahap1 = DB::table('tahap12020')->where('formal_name', '=', $lbg)->first();
            $tahap2 = DB::table('tahap22020')->where('formal_name', '=', $lbg)->first();
            $lulus1 = DB::table('lulus12020')->where('formal_name', '=', $lbg)->first();
            $lulus2 = DB::table('lulus22020')->where('formal_name', '=', $lbg)->first();
            $lulus2x = DB::table('kelulusan')->select('no_daftar')->where([
                ['status', '=', $lbg],
                ['tgl_lulus', '=', '01-07-2020'],
                ]);
            $santri = DB::table('siswa1')->wherein('no_daftar',$lulus2x)->
            orderby('jk','asc')->orderby('pil1','asc')->get();

            return view('admin.lembaga', compact('santri','lbg','tahap1','tahap2','lulus1','lulus2'));
    ...
    public function all()
    {
        $santri = DB::table('santri_profile')->get();
    ...
    public function petugas()
{
    ...
    public function persiapan()
    {
        ...
        $a = DB::table('kelulusan')->where('no_daftar',$user);
        $ao = $a->count();
        if($ao == 0){
            return view('test.persiapan', compact('p','date','d'));
        }else{
             if($date >= "01-07-2020 14:00:00"){
    ...
    public function lulus12020()
    {
        $lbg = Auth()->user()->id_jabatan;
        $santri = DB::table('kelulusan')
                    ->leftjoin('siswa1','kelulusan.no_daftar','=','siswa1.no_daftar')
                    ->where([
                        ['kelulusan.tgl_lulus','=','09-06-2020'],
                        ['kelulusan.status',$lbg],
                    ])->get();
        return view('admin.lulus12020',compact('santri'));
    }

    public function totalwil()
    {
        $total = DB::table('total_perwil')
                        ->orderby('kab','asc')
                        ->get();
        return view('admin.totalwil',compact('total'));
    }

    public function wil($kab)
    {
        $santri = DB::table('siswa1')
                        ->where('kab',$kab)
                        ->orderby('kab','asc')
                        ->get();
        return view('admin.editwil',compact('santri'));
    }
